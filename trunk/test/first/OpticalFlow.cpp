...
int maint(void)
{
	...
	CvCapture *input_video = cvCaptureFromFile("E:\\bysj\\test\\example2\\Hu\\wetherman\\run\\daria_run.avi");
	IplImage *frame0=0;
	frame0=cvQueryFrame(input_video);
	...
	while(true)
	{
		 ...
		 */
		cvCvtColor(frame, frame1_1C, CV_RGB2GRAY);//将frame垂直翻转后存储在frame1_1C
		cvNamedWindow("frame1_1C");
		cvShowImage("frame1_1C",frame1_1C);
		...
		cvCvtColor(frame,frame2_1C,CV_RGB2GRAY);
		cvNamedWindow("frame2_1C");
		...
		CvPoint2D32f frame1_features[FEATURE];
	...
	//	cvGoodFeaturesToTrack(frame1_1C, eig_image, temp_image, frame1_features, &number_of_features, .01, .01, NULL);
		move_img=cvCreateImage(cvSize(width,height),IPL_DEPTH_8U,1);
		cvZero(move_img);
		for (int i=0;i<width;i++)
       {
		   for (int j=0;j<height;j++)
		   {
			   double a=abs(cvGet2D(frame1_1C,j,i).val[0]-cvGet2D(frame2_1C,j,i).val[0]);
			   ...
			   if(a>=40)
			   {
                 if (p<FEATURE-1)
                 {
					 ...
					 int d=++p;
					 frame1_features[d].x=i;
					 frame1_features[d].y=j;
                 }
			   }
		   }
       }
		...
		CvPoint2D32f frame2_features[FEATURE];//保存在第一帧中的点在第二帧中的位置

		/* The i-th element of this array will be non-zero if and only if the i-th feature of
		 * frame 1 was found in frame 2.
		 */
		char optical_flow_found_feature[FEATURE];//frame 1 中的第i点出现在 frame 2 中，optical_flow_found_feature[i]就置为1

		/* The i-th element of this array is the error in the optical flow for the i-th feature
		 * of frame1 as found in frame 2.  If the i-th feature was not found (see the array above)
		 * I think the i-th entry in this array is undefined.
		 */
		float optical_flow_feature_error[FEATURE];
		...
		CvTermCriteria optical_flow_termination_criteria
			= cvTermCriteria( CV_TERMCRIT_ITER | CV_TERMCRIT_EPS, 64, .3 );
		...
		for(int i = 0; i <number_of_features; i++)
		{
			...
			cvLine(frame1,p,q,CV_RGB(255,0,0),1,8,0);
			cvLine(frame2_1C,p,q,CV_RGB(255,0,0),1,8,0);

			
/*

			double angle;		angle = atan2( (double) p.y - q.y, (double) p.x - q.x );//角度
			double hypotenuse;	hypotenuse = sqrt( square(p.y - q.y) + square(p.x - q.x) );//斜边*/

			/* Here we lengthen the arrow by a factor of three. 把箭头长度增大3倍*/ 
